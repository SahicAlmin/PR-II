...
/*
	...
	*/
string GenerisiOznaku(const char* imePrezime, int redniBroj, VrstaDostave vrstaDostave) {
	if (vrstaDostave < 0 || vrstaDostave>2 || redniBroj < 1 || redniBroj>9999) return "BH-XX-0000-XX";
...
public:
	...
	Kolekcija& DodajNaPoziciju(int lokacija, const T1& el1, const T2& el2) {
		...
		if (lokacija<0 || lokacija>_trenutno)throw exception("Lokacija nije vazeca\n");
	...
	pair<int, string> UkloniSaPozicije(int pozicija) {
		if (pozicija<0 || pozicija>=_trenutno) throw exception("Pozicija nije vazeca\n"); 
...
public:
	...
	string ToString()const {
		stringstream ss{}; 
		ss <<setfill('0')<<setw(2)<< * _dan << "." << setw(2)<< *_mjesec << "." << *_godina << " " <<setw(2)<<*_sati << ":" <<setw(2)<<*_minute << endl;
		return ss.str();
	}
...
public:
	Posiljka(const char* _sadrzaj, const char* imePrezime, const char* email, VrstaDostave vrsta, int broj, DatumVrijeme kreirana) {
		...
		vrstaDostave = vrsta; 
		_statusi.Dodaj(KREIRANA, kreirana); 
	}
...
*/
	bool DodajStatus(StatusPosiljke noviStatus, DatumVrijeme datum) {
		int trenutno = _statusi.GetTrenutno();
		StatusPosiljke zadnjiStatus = _statusi.GetElement1(trenutno - 1);
		DatumVrijeme zadnjeVrijeme = _statusi.GetElement2(trenutno - 1);

		// 1. Nakon ISPORUCENA ili OTKAZANA nije dozvoljena promjena
		if (zadnjiStatus == ISPORUCENA || zadnjiStatus == OTKAZANA)
			return false;

		// 2. Vrijeme mora biti veće od prethodnog
		if (!(datum > zadnjeVrijeme))
			return false;

		// 3. Ako je OTKAZANA, može iz bilo kojeg statusa osim završnih
		if (noviStatus == OTKAZANA) {
			_statusi.Dodaj(noviStatus, datum);
			return true;
		}

		// 4. Dozvoljen slijed statusa
		// KREIRANA -> PREUZETA -> U_TRANZITU -> ISPORUCENA
		if (zadnjiStatus == KREIRANA && noviStatus == PREUZETA) {
			_statusi.Dodaj(noviStatus, datum);
			return true;
		}
		if (zadnjiStatus == PREUZETA && noviStatus == U_TRANZITU) {
			_statusi.Dodaj(noviStatus, datum);
			return true;
		}
		if (zadnjiStatus == U_TRANZITU && noviStatus == ISPORUCENA) {
			_statusi.Dodaj(noviStatus, datum);
			return true;
		}

		// 5. Sve ostalo (preskakanje, ponavljanje, pogrešan slijed) nije dozvoljeno
		return false;
	}
	...
	string ToString()const {
		stringstream ss{}; 
		ss << GetOznaka() << "|" << GetImePrezimePrimaoca() << "|" << VrstaDostaveNazivi[GetVrstaDostave()] << "|" << GetSadrzaj() << "| " << StatusPosiljkeNazivi[GetTrenutniStatus()] << endl;
		return ss.str(); 
	}
...
//BrzaPosta Tim
void PosaljiObavjest(string email,string imePrezime,string oznaka,StatusPosiljke status,DatumVrijeme datum) {
	cout << crt; 
	cout << "TO:" << email << "\n";
	cout << "FROM:info@brzaposta.ba\n ";
	cout << "SUBJECT: PROMJENA STATUSA POSILJKE\n"; 
	cout << "Postovani " << imePrezime << "\n";
	cout << "Posiljka " << oznaka << " je promijenila status u " << status << "\n";
	cout << "Vrijeme evidentiranja: " << datum << "\n";
...
public:
	...
	*/
	void DodajPosiljku(const Posiljka& novaPosiljka) {
		...
		{
			if (strcmp(_posiljke[i].GetOznaka(), novaPosiljka.GetOznaka()) == 0 || _posiljke[i].GetRedniBroj() == novaPosiljka.GetRedniBroj()) {
				throw exception("Posiljka je vec dodana\n");
			}
		}
	...
	*/
	Posiljka* PronadjiPosiljku(string oznakaKnjige) {
		for (int i = 0; i < _posiljke.size(); i++)
		{
			if (strcmp(_posiljke[i].GetOznaka(), oznakaKnjige.c_str()) == 0) {
				return &_posiljke[i];
			}
		}
		return nullptr;
	}
	bool EvidentirajStatus(string oznakaKnjige, StatusPosiljke status, DatumVrijeme datum) {
		for (int i = 0; i < _posiljke.size(); i++)
		{
			if (strcmp(_posiljke[i].GetOznaka(), oznakaKnjige.c_str()) == 0) {
				if (_posiljke[i].DodajStatus(status, datum)) {
					string email = _posiljke[i].GetEmailPrimaoca();
					string imePrezime = _posiljke[i].GetImePrezimePrimaoca();
					thread t(PosaljiObavjest, email, imePrezime, oznakaKnjige, status, datum);
					t.detach();
					return true;
				}
				else {
	...
	vector<Posiljka*>PosiljkePoStatusu(StatusPosiljke status) {
		...
		for (int i = 0; i < _posiljke.size(); i++)
		{
			if (_posiljke[i].GetTrenutniStatus() == status) {
				rezultat.push_back(&_posiljke[i]);
			}
		}
		return rezultat; 
	}
	...
	*/
	Kolekcija<StatusPosiljke, int, 5> StatistikaPoStatusu() {
		Kolekcija<StatusPosiljke, int, 5> stat;
		for (int i = 0; i < 5; i++) {
...
int main() {
	...
	*/
	...
	cout << "Velicina vektora preuzetih: " << preuzetePosiljke.size() << endl;
