...
string GenerisiOznaku(const char* imePrezime, int redniBroj, int godina) {
	if (redniBroj < 1 || redniBroj>999 || godina < 2000 || godina>2099) {
...
bool ValidirajOznaku(const string& oznaka) {
	regex test("SRV-(?!000)\\d{3}/[A-Z]{2}-20\\d{2}");
...
public:
	...
	Kolekcija(const Kolekcija& obj) {
		...
		if (obj._elementi1 != nullptr && obj._elementi2 != nullptr) {
			_elementi1 = new T1[max];
			_elementi2 = new T2[max];
	...
	Kolekcija UkloniDuplikate() {
		...
		{
			bool duplikat = false;
			for (int j = 0; j < rezultat.GetTrenutno(); j++)
			{
				if (_elementi1[i] == rezultat.GetElement1(j)
					&& _elementi2[i] == rezultat.GetElement2(j)) {
					duplikat = true;
					break;
				}
			}
			if (!duplikat) {
				rezultat.Dodaj(_elementi1[i], _elementi2[i]);
			}
		}
		return rezultat;
	}
...
class Intervencija {
	...
	int _trajanjeMinuta;
public:
	Intervencija(const char* opis, const char* imePrezime, double cijena, int trajanje) {
		_opis = opis;
		_tehnicar = imePrezime;
		_cijena = cijena;
	...
	string ToString()const {
		stringstream ss;
		ss << GetOpis() << " | " << GetTehnicar() << " | " << GetCijena() << "KM | " << GetTrajanjeMinuta() << " min";
		return ss.str();
	}
...
public:
	ServisniZahtjev(const char* uredjaj, const char* opis, Klijent imePrezime, int broj, DatumVrijeme vrijeme) : _klijent(imePrezime) {
		_uredjaj = AlocirajTekst(uredjaj);
		_opisKvara = AlocirajTekst(opis);
		_redniBroj = broj;
		_oznaka = AlocirajTekst(GenerisiOznaku(_klijent.GetImePrezime().c_str(), _redniBroj, vrijeme.GetGodina()).c_str());
	...
	string ToString()const {
		stringstream ss;
		ss << GetOznaka() << " | " << GetKlijent().GetImePrezime() << " | " << GetUredjaj() << " | " << StatusZahtjevaNazivi[GetTrenutniStatus()];
		return ss.str();
	}
	...
	bool DodajStatus(StatusZahtjeva status, DatumVrijeme vrijeme) {
		DatumVrijeme novo = vrijeme;
		DatumVrijeme zadnjeVrijeme = _statusi.GetElement2(_statusi.GetTrenutno() - 1);
		StatusZahtjeva novi = status;
		StatusZahtjeva zadnji = GetTrenutniStatus();
		if (novo > zadnjeVrijeme) {
			if (zadnji == ZAVRSEN) {
				return false;
			}
			if (zadnji == ZAPRIMLJEN && novi == DIJAGNOSTIKA) {
				_statusi.Dodaj(novi, novo);
				return true;
			}
			if (zadnji == DIJAGNOSTIKA && novi == POPRAVKA) {
				_statusi.Dodaj(novi, novo);
				return true;
			}
			if (zadnji == POPRAVKA && novi == ZAVRSEN) {
				_statusi.Dodaj(novi, novo);
				return true;
			}
		}
	...
	double UkupnaCijena() {
		...
		{
			cijena += _intervencije[i].GetCijena();
		}
		return cijena;
	}
	int UkupnoTrajanje() {
		...
		for (int i = 0; i < _intervencije.size(); i++)
		{
			trajanje += _intervencije[i].GetTrajanjeMinuta();
		}
		return trajanje;
	}
	~ServisniZahtjev() {
...
void PosaljiRacun(string imePrezime, string email, string oznaka, string uredjaj, double cijena) {
	lock_guard<mutex>lock(mtx);
	cout << crt;
	cout << "To: " << email << "\n";
	cout << "From : racuni@servis.ba\n";
	cout << "Subject : Servisni zahtjev zavrsen - racun\n";
	cout << "Postovani " << imePrezime << "\n";
	cout << "Servisni zahtjev " << oznaka << " za uredjaj " << uredjaj << " je zavrsen\n";
	cout << "Ukupan iznos: " << cijena << "\n";
...
public:
	...
	void DodajZahtjev(const ServisniZahtjev& zahtjev) {
		...
		{
			if (strcmp(_zahtjevi[i].GetOznaka(), zahtjev.GetOznaka()) == 0 || _zahtjevi[i].GetRedniBroj() == zahtjev.GetRedniBroj()) {
				throw exception("Zahtjev je vec dodan\n");
			}
		}
	...
	ServisniZahtjev* PronadjiZahtjev(string oznaka) {
		for (int i = 0; i < _zahtjevi.size(); i++)
		{
			if (strcmp(_zahtjevi[i].GetOznaka(), oznaka.c_str()) == 0) {
				return &_zahtjevi[i];
			}
		}
		return nullptr;
	}
	bool EvidentirajStatus(string oznaka, StatusZahtjeva status, DatumVrijeme vrijeme) {
		for (size_t i = 0; i < _zahtjevi.size(); i++) {
			if (strcmp(_zahtjevi[i].GetOznaka(), oznaka.c_str()) == 0) {
				if (_zahtjevi[i].DodajStatus(status, vrijeme)) {
					if (status == ZAVRSEN) {
						string imePrezime = _zahtjevi[i].GetKlijent().GetImePrezime();
						string email = _zahtjevi[i].GetKlijent().GetEmail();
						string uredjaj = _zahtjevi[i].GetUredjaj();
						double ukupnaCijena = _zahtjevi[i].UkupnaCijena();

						thread t(PosaljiRacun, imePrezime, email, oznaka, uredjaj, ukupnaCijena);
						t.detach();
	...
	*/
	double IzracunajPrihod() {
		...
		for (int i = 0; i < _zahtjevi.size(); i++)
		{
			for (int j = 0; j < _zahtjevi[i].GetIntervencije().size(); j++)
			{
				if (_zahtjevi[i].GetTrenutniStatus() == ZAVRSEN) {
					prihod+=_zahtjevi[i].GetIntervencije()[j].GetCijena(); 
				}
			}
		}
		return prihod;
