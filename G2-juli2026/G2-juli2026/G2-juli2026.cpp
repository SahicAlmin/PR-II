...
string GenerisiOznaku(const char* imePrezime, int redniBroj, int godina) {
	if (redniBroj < 1 || redniBroj>999 || godina < 2000 || godina > 2099) return "SRV-000/XX-0000";
	string oznaka;
	oznaka += "SRV-";
	if (redniBroj < 10) {
		oznaka += "00";
...
public:
	...
	const T1& operator[](int index)const { return _elementi1[index]; }
...
public:
	ServisniZahtjev(const char* uredjaj, const char* opis, Klijent& klijent, int broj, DatumVrijeme& vrijeme)
	:_klijent(klijent){
		_uredjaj = AlocirajTekst(uredjaj); 
		_redniBroj = broj; 
		string oznaka = GenerisiOznaku(klijent.GetImePrezime().c_str(), _redniBroj, vrijeme.GetGodina()).c_str(); 
		_oznaka = AlocirajTekst(oznaka.c_str()); 
	...
	friend ostream& operator<<(ostream& COUT, const ServisniZahtjev& obj) {
		...
		*/
	bool DodajStatus(StatusZahtjeva noviStatus, DatumVrijeme& novoVrijeme) {
		StatusZahtjeva zadnjiStatus = GetTrenutniStatus();
		DatumVrijeme zadnjeVrijeme = _statusi.GetElement2(_statusi.GetTrenutno() - 1); 
		if (novoVrijeme > zadnjeVrijeme) {
			
		if (zadnjiStatus == ZAPRIMLJEN && noviStatus == DIJAGNOSTIKA) {
			_statusi.Dodaj(noviStatus, novoVrijeme); 
			return true;
		}
		if (zadnjiStatus == DIJAGNOSTIKA && noviStatus == POPRAVKA) {
			_statusi.Dodaj(noviStatus, novoVrijeme);
			return true;
		}
		if (zadnjiStatus == POPRAVKA && noviStatus == ZAVRSEN) {
			_statusi.Dodaj(noviStatus, novoVrijeme);
			return true;
		}
		}
