...
const char* GetDrzavaString(Drzava d) {
...
char* GenerisiID(string imePrezime, int broj) {
	...
	if (obrnutiBroj < 10) {
		id += "000";
	}else if (obrnutiBroj < 100) {
		id += "00"; 
	}
	else if(obrnutiBroj<1000)
	{
		id += "0";
	}
	id += std::toupper(imePrezime[imePrezime.find(" ") + 1]); 
	id += to_string(obrnutiBroj); 
	return GetNizKaraktera(id.c_str());
}
bool ValidirajID(string id) {
	regex testOne("[A-Z]{1}000[A-Z]{1}\\d{1}");
	regex testTwo("[A-Z]{1}00[A-Z]{1}\\d{2}");
	regex testThree("[A-Z]{1}0[A-Z]{1}\\d{3}");
	regex testFour("[A-Z]{2}\\d{4}");
	return regex_match(id, testOne)
		|| regex_match(id, testTwo)
		|| regex_match(id, testThree)
		|| regex_match(id, testFour); 
}
...
public:
	...
	Kolekcija RemoveRange(int from, int to) {
		if (from<0 || from>getTrenutno() || to<0 || to>getTrenutno()) throw exception("Range invalid\n"); 
		Kolekcija uklonjeni;
		Kolekcija preostali; 
		for (int i = 0; i < *_trenutno; i++)
		{
...
public:
	...
	friend ostream& operator<< (ostream& COUT, const Vrijeme& obj) {
		COUT <<setfill('0')<<setw(2)<< * obj._sat << ":" <<setw(2)<<*obj._minuta << ":" <<setw(2)<< *obj._sekunda;
		return COUT;
	}
};
class Pogodak {
...
void PosaljiEmailPojedinacnomIgracu(string idIgraca, string imeZabioca, int brojGola, Vrijeme v, string d1, string d2, int r1, int r2) {
	this_thread::sleep_for(chrono::seconds(2));

	lock_guard<mutex> lock(mtx); 
	cout << "To: " << idIgraca << "@euro2024.com" << endl;
	cout << "From: info@euro2024.com\nSubject: Informacija\nPostovani,\n";
	cout << "U " << v << " sati igrac " << imeZabioca << " je zabiljezio svoj " << brojGola << " pogodak na ovoj utakmici.\n";
	cout << "Trenutni rezultat je:\n" << d1 << " " << r1 << " : " << r2 << " " << d2 << "\n";
	cout << "Puno srece u nastavku susreta.\nNeka bolji tim pobijedi.\n";
	cout << "-------------------------------------------\n";
}

void PosaljiMailoveSvimIgracima(vector<Igrac> tim1, vector<Igrac> tim2, string imeZabioca, int brojGola, Vrijeme v, string d1, string d2, int r1, int r2) {
	for (size_t i = 0; i < tim1.size(); i++) {
		PosaljiEmailPojedinacnomIgracu(tim1[i].GetID(), imeZabioca, brojGola, v, d1, d2, r1, r2);
	}
	for (size_t i = 0; i < tim2.size(); i++) {
		PosaljiEmailPojedinacnomIgracu(tim2[i].GetID(), imeZabioca, brojGola, v, d1, d2, r1, r2);
	}
}
class Prventstvo {
...
public:
	...
	void AddUtakmicu(const Reprezentacija& rep1, const Reprezentacija& rep2) {
		for (int i = 0; i < _utakmice.getTrenutno(); i++) {
			if (_utakmice.getElement1(i).GetDrzava() == rep1.GetDrzava() && _utakmice.getElement2(i) == rep2.GetDrzava()) {
				throw exception("Utakmica vec odigrana\n"); 
				}
			if (_utakmice.getElement1(i).GetDrzava() == rep2.GetDrzava() && _utakmice.getElement2(i) == rep1.GetDrzava()) {
				throw exception("Utakmica vec odigrana\n");
			}
		}
		_utakmice.AddElement(rep1, rep2); 
	}
	bool AddPogodak(Drzava d1, Drzava d2, string idIliIme, Pogodak p) {
		for (int i = 0; i < _utakmice.getTrenutno(); i++) {
			Reprezentacija& r1 = _utakmice.getElement1(i);
			Reprezentacija& r2 = _utakmice.getElement2(i);

			if ((r1.GetDrzava() == d1 && r2.GetDrzava() == d2) || (r1.GetDrzava() == d2 && r2.GetDrzava() == d1)) {
				Igrac* trazeniIgrac = nullptr;
				for (size_t j = 0; j < r1.GetIgraci().size(); j++) {
					if (idIliIme == r1.GetIgraci()[j].GetID() || idIliIme == r1.GetIgraci()[j].GetImePrezime()) {
						trazeniIgrac = &r1.GetIgraci()[j];
						break;
					}
				}
				if (trazeniIgrac == nullptr) {
					for (size_t j = 0; j < r2.GetIgraci().size(); j++) {
						if (idIliIme == r2.GetIgraci()[j].GetID() || idIliIme == r2.GetIgraci()[j].GetImePrezime()) {
							trazeniIgrac = &r2.GetIgraci()[j];
							break;
						}
					}
				}

				if (trazeniIgrac == nullptr) return false;

				for (size_t j = 0; j < trazeniIgrac->GetPogoci().size(); j++) {
					if (strcmp(trazeniIgrac->GetPogoci()[j]->GetNapomena(), p.GetNapomena()) == 0)
						return false;
				}

				trazeniIgrac->GetPogoci().push_back(new Pogodak(p));

				int rez1 = 0, rez2 = 0;
				for (int i = 0; i < r1.GetIgraci().size(); i++) {
					rez1 += r1.GetIgraci()[i].GetPogoci().size();
				}
				for (int i = 0; i < r2.GetIgraci().size(); i++) {
					rez2 += r2.GetIgraci()[i].GetPogoci().size();
				}
				thread t(PosaljiMailoveSvimIgracima,
					r1.GetIgraci(),
					r2.GetIgraci(),
					string(trazeniIgrac->GetImePrezime()),
					trazeniIgrac->GetPogoci().size(),
					p.GetVrijemePogotka(),
					string(GetDrzavaString(r1.GetDrzava())),
					string(GetDrzavaString(r2.GetDrzava())),
					rez1,
					rez2
				);
	...
	vector<Igrac*> operator()(int minBroj) {
		vector<Igrac*>igraci; 
		for (int i = 0; i < _utakmice.getTrenutno(); i++)
		{
			for (int j = 0; j < _utakmice.getElement1(i).GetIgraci().size(); j++)
			{
				if (_utakmice.getElement1(i).GetIgraci()[j].GetPogoci().size() >= minBroj) {
					igraci.push_back(&_utakmice.getElement1(i).GetIgraci()[j]);
				}
			}
			for (int j = 0; j < _utakmice.getElement2(i).GetIgraci().size(); j++)
			{
				if (_utakmice.getElement2(i).GetIgraci()[j].GetPogoci().size() >= minBroj) {
					igraci.push_back(&_utakmice.getElement2(i).GetIgraci()[j]);
				}
			}
		}
		return igraci; 
	...
	friend ostream& operator<<(ostream& COUT, Prventstvo& obj) {
		for (int i = 0; i < obj._utakmice.getTrenutno(); i++) {
			...
			vector<string> strijelciDomacin;
			for (size_t j = 0; j < r1.GetIgraci().size(); j++) {
				for (size_t k = 0; k < r1.GetIgraci()[j].GetPogoci().size(); k++) {
					strijelciDomacin.push_back(r1.GetIgraci()[j].GetImePrezime());
				}
			}
			vector<string> strijelciGost;
			for (size_t j = 0; j < r2.GetIgraci().size(); j++) {
				for (size_t k = 0; k < r2.GetIgraci()[j].GetPogoci().size(); k++) {
					strijelciGost.push_back(r2.GetIgraci()[j].GetImePrezime());
				}
			}

			COUT << "-------------------------------------------\n";
			COUT << "(" << strijelciDomacin.size() << ") " << GetDrzavaString(r1.GetDrzava())
				<< " : " << GetDrzavaString(r2.GetDrzava()) << " (" << strijelciGost.size() << ")\n";
			COUT << "-------------------------------------------\n";

			size_t maxRedova = max(strijelciDomacin.size(), strijelciGost.size());
			for (size_t j = 0; j < maxRedova; j++) {
				
				if (j < strijelciDomacin.size()) {
					COUT << left << setw(30) << strijelciDomacin[j];
				}
				else {
					COUT << left << setw(30) << ""; 
				}
				if (j < strijelciGost.size()) {
					COUT << strijelciGost[j];
				}
				COUT << endl;
			}
			COUT << "-------------------------------------------\n";
