...
string GenerisiClanskiBroj(const char* imePrezime, int redniBroj) {
	...
	clanski_broj += std::toupper(inicijali[0]); 
	clanski_broj += std::toupper(inicijali[inicijali.find(" ") + 1]); 
	clanski_broj += "-";
	if (redniBroj < 10) {
		clanski_broj += "00"; 
	}
	...
	clanski_broj += to_string(redniBroj); 
	return clanski_broj; 
}
bool ValidirajClanskiBroj(const string& clanskiBroj) {
	regex test("\\d{4}/[A-Z]{2}-\\d{3}"); 
	return regex_match(clanskiBroj, test); 
}
...
public:
	...
	Kolekcija(const Kolekcija& obj) {
		...
		if (obj._prvi != nullptr && obj._drugi != nullptr) {
			_prvi = new T1[max];
			_drugi = new T2[max];
	...
	void Dodaj(const T1& el1, const T2& el2) {
		if (_prvi == nullptr || _drugi == nullptr) {
			_prvi = new T1[max]; 
			_drugi = new T2[max]; 
		}
		_prvi[_trenutno] = el1; 
	...
	Kolekcija& DodajNaPoziciju(int lokacija, const T1& el1, const T2& el2) {
		...
		if (lokacija > _trenutno || lokacija<0)throw exception("Lokacija nije vazeca\n"); 
		if (_prvi == nullptr || _drugi == nullptr) {
			_prvi = new T1[max]; 
			_drugi = new T2[max]; 
		 }
	...
	Kolekcija* UkloniRaspon(int from, int br) {
		if (from<0 || (from + br)>_trenutno) throw exception("Opseg nije validan\n"); 
		Kolekcija* uklonjeni = new Kolekcija(); 
		for (int i = from; i < (from+br); i++)
		{
			uklonjeni->Dodaj(_prvi[i], _drugi[i]); 
		}
		for (int i = from; i < _trenutno-br; i++)
		{
			_prvi[i] = _prvi[i + br];
			_drugi[i] = _drugi[i + br]; 
		}
		_trenutno -= br; 
		return uklonjeni; 
	}
...
class Posudba {
	...
	int _brojDana;
public:
	Posudba(const char* naslov, Zanr zanr, Datum datumPosudbe, int brojDana)
		: _zanr(zanr), _datumPosudbe(datumPosudbe), _brojDana(brojDana) {
	...
	string ToString() {
		...
		ss << setfill('0') << setw(2) << _datumPosudbe.GetDan()<<"."
			<<setfill('0')<< setw(2) << _datumPosudbe.GetMjesec()<<"."
			<< _datumPosudbe.GetGodina() << " "
			<< _naslov << " " << ZanrNazivi[_zanr] << " " << _brojDana << " dana" << endl;
		return ss.str(); 
	}
...
public:
	...
	bool DodajPosudbu(const Posudba& posudba) {
		...
		{
			if (strcmp(_posudbe[i].GetNaslov(), posudba.GetNaslov()) == 0) {
				return false; 
			}
		}
		if (posudba.GetBrojDana() > 30 || posudba.GetBrojDana() < 1) {
			return false; 
		}
		if (_posudbe.size() < 3) {
	...
	bool RazduziKnjigu(const char* naslovKnjige) {
		for (int i = 0; i < _posudbe.size(); i++)
		{
			if (strcmp(_posudbe[i].GetNaslov(), naslovKnjige) == 0) {
				return true; 
			}
		}
		return false; 
	...
	const vector<Posudba>& GetPosudbe()const { return _posudbe; }
...
public:
	...
	void DodajClana(const ClanBiblioteke& clan) {
		...
		{
			if (strcmp(_clanovi[i].GetClanskiBroj(), clan.GetClanskiBroj()) == 0) {
				throw exception("Clan vec dodan\n"); 
			}
		}
		_clanovi.push_back(clan); 
	}
	bool EvidentirajPosudbu(const char* clBroj, const Posudba& posudba) {
		for (int i = 0; i < _clanovi.size(); i++)
		{
			if (strcmp(_clanovi[i].GetClanskiBroj(), clBroj) == 0) {
				_clanovi[i].DodajPosudbu(posudba); 
				return true; 
			}
		}
		return false; 
	}
	vector<ClanBiblioteke*>AktivniClanovi(int minBr) {
		vector<ClanBiblioteke*>rezultat; 
		for (int i = 0; i < _clanovi.size(); i++)
		{
			if (_clanovi[i].GetPosudbe().size() >= minBr) {
				rezultat.push_back(&_clanovi[i]); 
			}
		}
		return rezultat; 
	}
	Kolekcija<ClanBiblioteke, int, 50>PosudbePoZanru(Zanr zanr) {
		Kolekcija<ClanBiblioteke, int, 50> par; 
		for (int i = 0; i <_clanovi.size(); i++)
		{
			int brojac = 0; 
	...
	const vector<ClanBiblioteke>& GetClanovi() const{ return _clanovi; }
};
Biblioteka* pronadjiBiblioteku(string naziv, vector<Biblioteka>& biblioteke) {
	for (int i = 0; i < biblioteke.size(); i++)
	{
		if (strcmp(biblioteke[i].GetNaziv(), naziv.c_str()) == 0) {
			return &biblioteke[i]; 
		}
	}
	biblioteke.push_back(Biblioteka(naziv.c_str())); 
	return &biblioteke.back(); 
}
bool postojiLiClan(string imePrezime, Biblioteka biblioteka) {
	for (int i = 0; i < biblioteka.GetClanovi().size(); i++)
	{
		if (strcmp(biblioteka.GetClanovi()[i].GetImePrezime(), imePrezime.c_str()) == 0) {
			return true; 
		}
	}
	return false; 
}
bool UcitajPodatke(string filePath, vector<Biblioteka>& biblioteke) {
	...
	while (getline(file, line)) {
		...
		string imePrezime; 
		getline(streamLine, nazivBiblioteke, '|'); 
		getline(streamLine, imePrezime); 
		Biblioteka* biblioteka = pronadjiBiblioteku(nazivBiblioteke, biblioteke); 
		if (!postojiLiClan(imePrezime, *biblioteka)) {
			biblioteka->DodajClana(ClanBiblioteke(imePrezime.c_str())); 
		}
	}
	file.close(); 
	return true; 
...
//	cout << "Pitanje -> Pojasnite kako tip nasljedjivanja (public, protected, private) utice na dostupnost clanova bazne klase u izvedenoj klasi. Sta se desava sa public i protected clanovima bazne klase ako se nasljedjivanje izvrsi kao protected ili private?.\n";
...
int main() {
	...
	inventar.DodajNaPoziciju(21,21,"Posebno izdanje");
}
catch (exception& e) {
	...
	for (auto clan : aktivni)
		cout << clan->GetImePrezime() << " ima " << clan->GetPosudbe().size()
